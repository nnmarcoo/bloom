...
fn fs_main(in: VertexOut) -> @location(0) vec4f {
    ...
    let pass_through_v = in.uv.y;

    if texels_per_pixel <= 1.0 {
        return textureSampleLevel(source, tex_sampler, vec2f(in.uv.x, pass_through_v), 0.0);
    }
    ...
    for (var i = -radius; i <= radius; i++) {
        ...
        color += textureSampleLevel(source, tex_sampler, vec2f(sample_u, pass_through_v), 0.0) * weight;
        weight_sum += weight;
    }
