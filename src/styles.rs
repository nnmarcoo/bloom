...
fn mix_bias(a: Color, b: Color, t: f32) -> Color {
    let t = t * t;
    Color {
        r: a.r + (b.r - a.r) * t,
        g: a.g + (b.g - a.g) * t,
...
pub fn icon_button_rotate_style(
    rotation: usize,
) -> impl Fn(&Theme, button::Status) -> button::Style {
    move |theme: &Theme, status: button::Status| {
        let palette = theme.extended_palette();

        let anchor = palette.primary.strong.color;

        let rotate_color = match rotation % 4 {
            0 => None,
            1 => Some(mix_bias(anchor, Color::from_rgb(1.0, 0.85, 0.0), 0.45)), // yellow
            2 => Some(mix_bias(anchor, Color::from_rgb(1.0, 0.55, 0.0), 0.55)), // orange
            3 => Some(mix_bias(anchor, Color::from_rgb(0.9, 0.2, 0.2), 0.65)),  // red
            _ => unreachable!(),
        };
        ...
        let background = match status {
            button::Status::Hovered => Some(Background::Color(palette.background.base.color)),
            button::Status::Pressed => Some(Background::Color(palette.background.weak.color)),
            _ => rotate_color.map(Background::Color),
        };
